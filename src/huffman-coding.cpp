 ...
 * 
 * @param v Vector containing pairs of lengths of codes and characters.
 */
...
std::string *HCEncode(std::string const &input)
{
    ...
    std::priority_queue<int, std::vector<int>, CompareWeight> q;
    for (int i = 0; i < ALPHABET_SIZE; ++i)
    {
...
std::string HCDecode(std::string const &input)
{
    ...
    std::string output = "";
    int current_node = 0;
    for (int i = ALPHABET_SIZE; i < (int)input.length(); ++i)
    {
        uint8_t x = input[i];
        for (int k = 7; k >= 0; --k)
        {
